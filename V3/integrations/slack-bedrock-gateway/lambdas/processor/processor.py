...
def get_current_date_context() -> str:
    ...
    Returns formatted date context string to inject into all agent requests
    """
    now = datetime.now(timezone.utc)
    
    # Format comprehensive date context
    date_context = f"""
...
**IMPORTANT**: Use this current date information to interpret all time-based references in the user's request. When they say "this quarter", "recent", "last month", etc., calculate these relative to the current date above.
...
def invoke_bedrock_agent(user_message: str, session_id: str, channel_id: str = None, message_ts: str = None, thread_ts: str = None) -> str:
    ...
    try:
        ...
        logger.info(f"Agent ID: {BEDROCK_AGENT_ID}, Alias ID: {BEDROCK_AGENT_ALIAS_ID}")
        ...
        logger.info(f"Got response from Bedrock Agent: {list(response.keys())}")
        
        # Process the response stream
        agent_response = ""
        completion_events = []
        last_progress_time = time.time()
        progress_throttle = 0  # No throttling for debugging - show all traces
        ...
        for event in response['completion']:
            ...
            if 'chunk' in event:
                ...
                if 'bytes' in chunk:
                    ...
                    logger.info(f"Received chunk #{chunk_count}: {chunk_text[:50]}...")
                else:
                    logger.info(f"Chunk #{chunk_count} has no bytes: {list(chunk.keys())}")
                ...
                logger.info(f"Trace event #{trace_count} received with keys: {list(trace.keys())}")
                if 'orchestrationTrace' in trace:
                    orch_keys = list(trace['orchestrationTrace'].keys())
                    logger.info(f"Orchestration trace keys: {orch_keys}")
                    # Log the actual content for debugging
                    orch_trace = trace['orchestrationTrace']
                    if 'rationale' in orch_trace:
                        rationale_text = orch_trace['rationale'].get('text', '')[:100]
                        logger.info(f"Found rationale: {rationale_text}...")
                    if 'invocationInput' in orch_trace:
                        logger.info(f"Found invocationInput with keys: {list(orch_trace['invocationInput'].keys())}")
                    if 'observation' in orch_trace:
                        logger.info(f"Found observation with keys: {list(orch_trace['observation'].keys())}")
                else:
                    logger.info("No orchestrationTrace found in trace event")
                    # Log what we do have in the trace
                    for key, value in trace.items():
                        logger.info(f"Trace contains {key}: {str(value)[:100]}...")
        ...
        logger.info(f"Stream processing complete. Events: {event_count}, Traces: {trace_count}, Chunks: {chunk_count}")
        ...
        logger.info(f"Final response preview: {agent_response[:200]}...")
...
def process_app_mention(event_data: Dict[str, Any]) -> bool:
    ...
    try:
        ...
        enhanced_message = f"{date_context}**USER REQUEST:**\n{message_text}"
        
        logger.info(f"Injected date context into message. Enhanced message length: {len(enhanced_message)}")
        
        # Invoke Bedrock Agent with session management and progress tracking
        logger.info(f"About to invoke Bedrock Agent with session_id: {session_id}")
        ...
        logger.info("Starting Slack message sending process...")
        success = False
        ...
        logger.info(f"process_app_mention returning: {success}")
        return success
        
    except KeyError as e:
        logger.error(f"Missing required field in event_data: {str(e)}", exc_info=True)
        logger.error(f"Event data keys available: {list(event_data.keys())}")
        return False
    except Exception as e:
