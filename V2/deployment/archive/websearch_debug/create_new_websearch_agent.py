#!/usr/bin/env python3
"""
Create New WebSearch Agent
==========================

Creates a completely new WebSearch Agent from scratch to replace the broken one.
"""
...
def create_new_websearch_agent():
    """Create a brand new WebSearch Agent from scratch"""
    
    logger.info("🚀 Creating New WebSearch Agent from Scratch")
    logger.info("=" * 60)
...
ALWAYS call functions first, then analyze the results.'''
    ...
    try:
        ...
        agent_response = bedrock_agent.create_agent(
            agentName="revops-websearch-agent-v2",
            description="WebSearch Agent V2 - Clean implementation for external intelligence gathering",
        ...
        # Step 2: Create action group with proper schema
        ...
        alias_response = bedrock_agent.create_agent_alias(
            agentId=new_agent_id,
            agentAliasName="revops-websearch-v2-alias",
            description="Production alias for WebSearch Agent V2"
        )
        ...
        test_response = bedrock_runtime.invoke_agent(
            ...
            inputText="Call your search_web function to search for 'WINN.AI company'"
        )
...
def main():
    ...
    if new_config:
        ...
        if config_updated:
            ...
            logger.info("  4. Update other agents to use new WebSearch Agent ID if needed")
        else:
